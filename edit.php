...
require_capability('enrol/ilbeadtutorado:config', $context);

$PAGE->set_url('/enrol/ilbeadtutorado/edit.php', array('courseid'=>$course->id, 'id'=>$instanceid));
...
/** @var enrol_ilbeadtutorado_plugin $plugin */
$plugin = enrol_get_plugin('ilbeadtutorado');

if ($instanceid) {
    $instance = $DB->get_record('enrol', array('courseid'=>$course->id, 'enrol'=>'ilbeadtutorado', 'id'=>$instanceid), '*', MUST_EXIST);
...
$mform = new enrol_ilbeadtutorado_edit_form(NULL, array($instance, $plugin, $context));
...
$PAGE->set_title(get_string('pluginname', 'enrol_ilbeadtutorado'));

echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('pluginname', 'enrol_ilbeadtutorado'));
$mform->display();
echo $OUTPUT->footer();
